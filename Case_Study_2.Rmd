...
## 1.2 Anti-image correlation, KMO and Bartlett's test
...
The Kaiser-Meyer-Olkin criterion suggest that the adequacy of the data is marvelous. 
...
plot(fa_result$values,xlab="Factor Number",ylab="Eigenvalue",main="Scree plot",cex.lab=1.2,cex.axis=1.2,cex.main=1.8)+abline(h=1)

factors_kaiser <- sum(fa_result$values>1)
```

Kaiser criterion gives 5 factors, scree-test  gives 10

```{r}
FA_communalities=data.frame(sort(fa_result$communality))
problematic_communalities <- rownames(FA_communalities)[FA_communalities<0.5]
problematic_communalities
```

There are two communalities which are below 0.5. We need to watch out for these two. 
